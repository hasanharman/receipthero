...
const createThumbnail = (
  ...
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      console.error("❌ Failed to create thumbnail, using placeholder");
...
export function useReceiptManager() {
    ...
    async (
      ...
      try {
        ...
        for (const receipt of receipts) {
          if (
            receipt.thumbnail &&
            receipt.thumbnail !== "/placeholder.svg" &&
            !receipt.thumbnail.startsWith("/")
          ) {
        ...
        console.log("💾 Saved to storage:", receipts.length, "receipts");
      } catch (error) {
    ...
    async (files: File[]): Promise<UploadedFile[]> => {
      ...
      const filePromises = files.map(async (file) => {
        try {
          ...
          console.log(`📤 Sending ${file.name} to OCR API...`);

          // Create FormData and send PDF file
          ...
          console.log(`📥 OCR Response for ${file.name}:`, data);
          ...
          if (response.ok && data.receipts && data.receipts.length > 0) {
            ...
            console.log(`🖼️ Creating thumbnail for ${file.name}...`);
            ...
            console.log(
              `✅ Processed file created for ${file.name}:`,
              processedFile
            );
            return processedFile;
          } else {
            console.log(`⚠️ No receipt data found in ${file.name}`);
      ...
      console.log("✅ Final processed files:", results);
      return results;
    },
    [addToast]
  );
  ...
  const addReceipts = useCallback(
    async (uploadedFiles: UploadedFile[]) => {
      console.log("➕ Adding receipts from", uploadedFiles.length, "file(s)");
      ...
      try {
        ...
        const newReceipts = receiptFiles.filter(
          (newReceipt) =>
        ...
        console.log("🆕 New receipts:", newReceipts.length);
        console.log("🔄 Duplicates:", duplicatesCount);

        if (newReceipts.length === 0) {
          console.log("⚠️ No new receipts to add");
        ...
        console.log("💾 Saving", updatedReceipts.length, "total receipts");

        setReceipts(updatedReceipts);
        setBreakdown(newBreakdown);
        await saveToStorage(updatedReceipts, newBreakdown);

        console.log("✅ Receipts added successfully");
  ...
  const deleteReceipt = useCallback(
    async (receiptId: string) => {
      ...
      if (updatedReceipts.length === 0) {
        console.log("📭 No receipts left, clearing all data");
        setReceipts([]);
        setBreakdown(null);
        ...
        setBreakdown(newBreakdown);
        await saveToStorage(updatedReceipts, newBreakdown);
        console.log("✅ Receipt deleted, remaining:", updatedReceipts.length);
      }
    },
